class VentaManager {
    ...
    async validarStockProducto() {
        ...
        try {
            const response = await fetch(`/Ventas/ValidarStock?productoId=${productoId}&cantidad=${cantidad}`);
            const result = await response.json();

            const btnAgregar = $('#agregarProducto');
            if (result.tieneStock) {
                btnAgregar.prop('disabled', false).removeClass('btn-danger').addClass('btn-success');
            } else {
                btnAgregar.prop('disabled', true).removeClass('btn-success').addClass('btn-danger');
    ...
    async agregarProducto() {
        ...
        const productoExistente = this.productosEnVenta.find(p => p.productoId === productoId);
        if (productoExistente) {
            alert('El producto ya está en la venta. Use la opción de editar cantidad.');
            return;
        }

        try {
            const response = await fetch(`/Ventas/ObtenerProducto?productoId=${productoId}`);
            const result = await response.json();
    ...
    async procesarVenta() {
        ...
        const response = await fetch('/Ventas/CreateJson', { // ← Cambiar URL
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'RequestVerificationToken': $('input[name="__RequestVerificationToken"]').val()
    ...
    limpiarSeleccionProducto() {
        $('#productoSelect').val('');
        $('#cantidad').val(1);
        $('#agregarProducto').prop('disabled', false).removeClass('btn-danger').addClass('btn-success');
    }
